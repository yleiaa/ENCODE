...
if args.target is not None: #If Not Control
    ...
    searchURL+='&target.label%21=Control'
    searchURL+='&assay_title=TF+ChIP-seq'
else: #If Control
    searchURL+='&assay_title=Control+ChIP-seq' 
    searchURL+='&target.investigated_as=control'
    searchURL+='&target.label=Control' 

searchURL+='&biosample_ontology.term_name='+args.biosample 
searchURL+='&files.file_type=fastq'
